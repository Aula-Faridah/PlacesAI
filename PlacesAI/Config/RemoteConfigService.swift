...
class RemoteConfigService: RemoteConfigurable {
    ...
    func fetchConfig<T: Decodable>(forKey key: ConfigKeys) async throws -> T {
        ...
        ///Pastikan bahwa nilai dari config value yang kita set di Firebase remote config itu berupa String
        ...
        ///Jika ternyata ketika didecode, generic T itu bukan sebuah String, kita guard agar tidak crash
        
        guard let data = stringValue.data(using: .utf8) else {
            throw ErrorConfiguration.failedToConvertData(key: key.rawValue)
            //            print("Failed to convert configuration key data")
            //            return
        }
...
//Implement Firebase RemoteConfigService
